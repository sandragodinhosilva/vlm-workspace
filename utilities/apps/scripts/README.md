...
| `snapshot_sft_config.py` | Snapshot an SFT config into `nemo-rl-vlm/logs/<run>/config_snapshot.yaml` at launch, so the `sft-dashboard` (port 7875) config panel shows config-as-run. Called by the `/vlm-launch-sft` workflow right after config verification. Run with the nemo `.venv`; see `--help`. |
