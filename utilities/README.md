...
| [`eval/compile_eval_results.py`](eval/compile_eval_results.py) | Compile results from all three pipelines (aux, benchmarks, visualobs) into one master CSV at `/mnt/data/sgsilva/results/eval_master/eval_master.csv`. Additive — originals untouched. Re-run anytime; rebuilds from scratch. | any | `vlm-post-training-home-venv/bin/python` |
